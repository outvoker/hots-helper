...
"""Vendored copy of ``heroprotocol.decoders`` with the ``six`` dependency
removed and rewritten for pure Python 3.

We maintain our own copy so that the frozen Windows .exe doesn't need
``six`` (a transitive dep) or ``heroprotocol`` (whose ``__init__`` chain
imports the removed ``imp`` module on Python 3.12+) at runtime. Without
this, loading any vendored ``protocolXXXXX.py`` blows up at exec time
and the resulting empty module shell triggers
``module hots_helper_protocol_… has no attribute decode_replay_header``.
"""
...
def _ensure_binary(s, encoding='utf-8') -> bytes:
...
def _byte2int(b) -> int:
    """Subscripting bytes returns an int in py3, but the original
    heroprotocol code passed a 1-byte slice through ``six.byte2int``.
    Accept either form."""
    if isinstance(b, int):
        return b
    return b[0]
...
class BitPackedBuffer:
    ...
    def __str__(self):
        s = (
            '{:02x}'.format(_byte2int([self._data[self._used]]))
    ...
    def read_aligned_bytes(self, bytes):
        self.byte_align()
        data = _ensure_binary(self._data[self._used : self._used + bytes])
        self._used += bytes
        if len(data) != bytes:
    ...
    def read_bits(self, bits):
        ...
        while resultbits != bits:
            if self._nextbits == 0:
                if self.done():
                    raise TruncatedError(self)
                self._next = _byte2int([self._data[self._used]])
                self._used += 1
                self._nextbits = 8
